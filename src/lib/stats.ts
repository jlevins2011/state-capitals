...
export function accuracyTrend(child: Child): { t: number; accuracy: number }[] {
  return [...child.sessions]
    .sort((a, b) => a.startedAt - b.startedAt)
    .map((session) => ({ t: session.startedAt, accuracy: session.accuracy }));
}

export function weakStates(child: Child, limit = 6): { key: string; misses: number }[] {
  const tally: Record<string, number> = {};
  for (const session of child.sessions) {
    for (const [key, n] of Object.entries(session.stateErrors)) {
...
export function isCompassKeeper(child: Child): boolean {
  const exam = child.completedLessons["atlas-exam"];
